...
DEVICE
__half getTensor( __half* tensor, int xi, int yi )
{
	return tensor[xi * SHARED_TENSOR_ROW + yi];
}
DEVICE
void setTensor( __half* tensor, int xi, int yi, __half value )
{
	tensor[xi * SHARED_TENSOR_ROW + yi] = value;
}
...
extern "C" __global__ void forwardWMMA( float* intermediates, __half* matBuffer, float* matBufferF, MLPForwardArg arg )
{
    ...
    if( arg.encoder == EncoderType::None )
    {
		...
		{
			int yi = yi_global_base + yi_local;
			if( xi < arg.inputMat.m_col && yi < arg.inputMat.m_row )
			{
				vs[yi_local] = intermediates[elem( xi, yi, arg.inputMat )];
			}
			else
	...
	for( int i = 0; i < arg.nLayer; i++ )
	{
        ...
        if( x_location < col ) // check if this wave has work
		{
...
extern "C" __global__ void avg( float* occupancyGrid, float* occupancyAverage )
{
    ...
    float density = 0.0f;
	for( int i = 0; i < NERF_AVG_BATCH; i++ )
	{
		density += occupancyGrid[x * NERF_AVG_BATCH + i];
	}
